<?php if($_settings->userdata('type') == 1): ?>
    <!-- Đây là phần hiển thị dành cho admin -->
    <h2><?php echo $_settings->info('name') ?> - Trang Quản Lý</h2>
<hr>
<div class="row">
...
<?php else: ?>
    <!-- Đây là phần dành cho thành viên -->
    <h2><?php echo $_settings->info('name') ?></h2>
    <hr>
    <!-- Hiển thị các thông tin dành cho thành viên, ẩn một số mục admin -->
    <div class="row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
            <div class="info-box">
                <span class="info-box-icon bg-gradient-secondary elevation-1"><i class="fas fa-th-list"></i></span>
                <div class="info-box-content">
                    <span class="info-box-text">Tổng số sân</span>
                    <span class="info-box-number text-right">
                        <?php 
                          $court = $conn->query("SELECT * FROM court_list where `delete_flag` = 0 and `status` = 1")->num_rows;
                          echo format_num($court);
